...
-- Add max_items_per_feed column to user_rss_feeds
ALTER TABLE public.user_rss_feeds ADD COLUMN max_items integer NOT NULL DEFAULT 5;

-- Update get_river_entries to limit RSS items per feed and fix public filter
...
BEGIN
  ...
  ELSIF p_filter = 'all' AND p_viewer_id IS NOT NULL THEN
    -- Mix xcrol entries + RSS (limited per feed)
    RETURN QUERY
    (
      SELECT
    ...
    (
      ...
      FROM (
        SELECT
          ri.id, ri.title AS content, ri.link,
          (ri.published_at AT TIME ZONE 'UTC')::date AS entry_date,
  ...
  ELSIF p_filter = 'public' THEN
    -- Public and Above: all posts the viewer is authorized to see (no RSS)
    RETURN QUERY
    SELECT
      e.id, e.content, e.link, e.entry_date, e.privacy_level, e.user_id,
      p.display_name, p.avatar_url, p.username
    ...
    LIMIT p_limit OFFSET p_offset;
  ELSE
    -- Original behavior for friendship-level filters (or anonymous 'all')
    RETURN QUERY
    SELECT
      e.id, e.content, e.link, e.entry_date, e.privacy_level, e.user_id,
      p.display_name, p.avatar_url, p.username
