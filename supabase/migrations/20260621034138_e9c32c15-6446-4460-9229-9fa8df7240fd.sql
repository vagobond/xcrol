...
CREATE OR REPLACE FUNCTION public.get_river_replies(p_entry_ids uuid[], p_viewer_id uuid DEFAULT NULL::uuid)
RETURNS TABLE(id uuid, entry_id uuid, user_id uuid, content text, created_at timestamp with time zone, display_name text, avatar_url text, username text, can_view_content boolean, parent_reply_id uuid)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WITH RECURSIVE base AS (
    SELECT
      ...
      r.created_at,
      r.parent_reply_id,
      e.privacy_level AS entry_privacy,
      CASE
        -- Replies on PUBLIC parent entries are always visible to everyone
        WHEN e.privacy_level = 'public' THEN true
    ...
    JOIN xcrol_entries e ON e.id = r.entry_id
